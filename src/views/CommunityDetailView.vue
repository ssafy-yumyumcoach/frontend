    ...
    try {
        ...
        if (newFiles.length > 0) {
            const uploadPromises = newFiles.map(file => 
                imageApi.uploadImage(file, 'POST')
            );
...
<template>
  <div class="space-y-6 max-w-2xl mx-auto">
    ...
    <div v-else-if="post" class="bg-zinc-900 border border-zinc-800 rounded-xl overflow-hidden p-6">
        ...
        <div v-if="!isEditMode">
            ...
            <div v-if="post.images && post.images.length > 0" class="space-y-4 mb-6">
                <div v-for="(img, idx) in post.images" :key="idx" class="rounded-xl overflow-hidden bg-zinc-800 border border-zinc-700">
        ...
        <div v-else class="space-y-4">
            ...
            <div class="space-y-2">
                ...
                <div class="grid grid-cols-4 gap-2 mb-2">
                    <div v-for="(img, idx) in editImages" :key="idx" class="relative aspect-square rounded-lg overflow-hidden bg-zinc-800">
    ...
    <div v-if="post && !isEditMode" class="bg-zinc-900 border border-zinc-800 rounded-xl p-6 space-y-6">
        ...
        <!-- List -->
        <div class="space-y-4">
             <div v-if="comments.length === 0" class="text-center py-6 text-zinc-500">
                작성된 댓글이 없습니다.
             </div>
               ...
               <div class="flex-1 bg-zinc-800/50 rounded-lg p-3">
                 ...
                 <div v-if="editingCommentId === comment.commentId" class="mt-2 space-y-2">
                     ...
                     <div class="flex justify-end gap-2">
                         <Button size="icon" variant="ghost" @click="cancelEditComment" class="h-6 w-6 text-zinc-400 hover:text-white">
                             <X class="w-3 h-3" />
                         </Button>
                         <Button size="icon" variant="ghost" @click="handleUpdateComment(comment.commentId)" class="h-6 w-6 text-emerald-500 hover:text-emerald-400">
                             <Send class="w-3 h-3" />
                         </Button>
                     </div>
                 </div>
